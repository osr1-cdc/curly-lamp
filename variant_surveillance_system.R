...
node="08" #you can use any node from 08 - 13. Sometimes nodes fail so if you get an error you can try another node
#-------------- Import Data ---------------
...
impala <- DBI::dbConnect(drv = drv, 
                         paste0("jdbc:impala://cdp-", 
                                node,".biotech.cdc.gov:21050/sc2_air;AuthMech=3;useSasl=1;SSL=1;AllowSelfSignedCerts=1;CAIssuedCertNamesMismatch=1"),
                         opts$user, 
                         opts$password)
...
query = paste('SELECT', 
              ...
              'WHERE A.primary_country in ("United States", "USA")') # if unavailable, use test_deduplication_cdcncbigisaid_auto for testing
...
tests = dbGetQuery(impala, paste0(
...
save(dat, pangolin, baseline, tests, pops, file = paste0(script.basename, "/data/", "variant_survey_dat", data_date, custom_tag, ".RData")) # "surveillance/variant_survey_dat.RData")
...
#
save(svy.dat, file = paste0(script.basename, "/data/", "svydat_", data_date, custom_tag, ".RData"))
